...
PLY layout is the 2D-GS training format's own contract (not viewer-specific):
`f_rest_` properties are laid out `[N, 3*K]` with axis-1 ordered
`[R_0..R_{K-1}, G_0..G_{K-1}, B_0..B_{K-1}]`, so the reshape must be
`reshape(-1, 3, K).transpose(0, 2, 1)` -- reshaping directly to `(-1, K, 3)`
silently scrambles the RGB channels into the wrong colors. Quaternions are
stored `(w, x, y, z)`. Note `scale_` may have 2 or 3 properties -- 2D-GS
surfels only need 2 (in-plane) scale axes, unlike 3D-GS's 3.
...
def _sanitize(arr: np.ndarray, label: str) -> np.ndarray:
    ...
    if n_bad > 0:
        print(f"[gs_sensor_core] {label}: zeroing {n_bad:,} NaN/Inf values")
