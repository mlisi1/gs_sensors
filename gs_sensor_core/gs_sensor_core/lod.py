...
the leaf's projected screen size is small (see
GaussianRasterizerWrapper.render's leaf_fine/leaf_coarse split in
render/rasterizer.py). Split out from culling.py because proxy
construction (moment-matching, quaternion math) is a distinct concern
from octree building/frustum culling and only touches Octree's proxy_*
fields, not its spatial-partitioning core.
"""
...
def _quat_to_rotmat_batch(q_wxyz: np.ndarray) -> np.ndarray:
    ...
    3, 3] -- this model's rotation convention (see gaussian_model.py), not
    rotations.py's (x, y, z, w) pose convention, and not the same thing.
    Same formula as rotations.quat_to_rotmat (verified equivalent, given a
    reordered input, in the correctness tests for build_leaf_proxies) and
    independently cross-checked against the CUDA kernel's own
    auxiliary.h::quat_to_rotmat -- written separately here (rather than
    reusing rotations.py) only because that one isn't batched and this
    needs to run over many points at once without a Python-level loop."""
...
def build_leaf_proxies(
    ...
    - color: weighted average of the DC (view-independent) term only;
      ...
      behavior, the same tradeoff compression_level=3 makes for the whole
      model.
    ...
    from gs_sensor_core.rotations import rotmat_to_quat
